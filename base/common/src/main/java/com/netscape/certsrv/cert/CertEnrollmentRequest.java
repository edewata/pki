...
public class CertEnrollmentRequest extends ResourceMessage {
    ...
    public Element toDOM(Document document) throws Exception {
        ...
        for (ProfileOutput output: getOutputs()) {
            ...
            certEnrollmentRequestElement.appendChild(output.toDOM(document));

            NodeList nodes = certEnrollmentRequestElement.getElementsByTagName("profileOutput");
            System.out.println(nodes.getLength());
            for (int i=0; i < nodes.getLength(); i++) {
                document.renameNode(nodes.item(i), null, "Output");
            }
