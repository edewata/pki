...
public class SecurityDomainHostAddCLI extends CommandCLI {
    ...
    public void createOptions() {
        Option option = new Option(null, "unsecurePort", true, "Unsecure port (default: 8080)");
        option.setArgName("port");
        options.addOption(option);

        option = new Option(null, "eePort", true, "EE port (default: 8443)");
        option.setArgName("port");
        options.addOption(option);

        option = new Option(null, "adminPort", true, "Admin port (default: 8443)");
        option.setArgName("port");
        options.addOption(option);

        option = new Option(null, "agentPort", true, "Agent port (default: 8443)");
        option.setArgName("port");
        options.addOption(option);
    ...
    public void execute(CommandLine cmd) throws Exception {
        ...
        String unsecurePort = cmd.getOptionValue("unsecurePort", "8080");
        host.setPort(unsecurePort);

        String eePort = cmd.getOptionValue("eePort", "8443");
        host.setSecureEEClientAuthPort(eePort);

        String adminPort = cmd.getOptionValue("adminPort", "8443");
        host.setSecureAdminPort(adminPort);

        String agentPort = cmd.getOptionValue("agentPort", "8443");
        host.setSecureAgentPort(agentPort);
