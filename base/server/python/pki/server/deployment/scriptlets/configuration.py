...
class PkiScriptlet(pkiscriptlet.AbstractBasePkiScriptlet):
    ...
    def import_system_certs(self, deployer, nssdb, subsystem):
        ...
        if subsystem.name == 'ca':
            ...
            ca_signing_nickname = deployer.mdict['pki_ca_signing_nickname']
            self.import_ca_signing_cert(deployer, nssdb)

        if subsystem.name in ['kra', 'ocsp']:

            ca_signing_nickname = deployer.mdict['pki_cert_chain_nickname']
        ...
        # NSS may override CA signing cert trust flags while importing other
        # PKCS #7 certs, so the flags need to be reset.
        nssdb.modify_cert(
            nickname=ca_signing_nickname,
            trust_attributes='CT,C,C')
