...
class CACertAddCLI(pki.cli.CLI):

    def __init__(self):
        super(CACertAddCLI, self).__init__(
            'add', 'Add certificate into CA')

    def print_help(self):
        print('Usage: pki-server ca-cert-add [OPTIONS]')
        print()
        print('  -i, --instance <instance ID>       Instance ID (default: pki-tomcat).')
        print('      --cert <path>                  Certificate path')
        print('      --format <format>              Certificate format: PEM (default), DER.')
        print('      --profile <profile ID>         Certificate profile ID')
        print('  -v, --verbose                      Run in verbose mode.')
        print('      --debug                        Run in debug mode.')
        print('      --help                         Show help message.')
        print()
    ...
    def execute(self, argv):
        ...
        if not cert_path:
            logger.error('Missing certificate to add')
            self.print_help()
            sys.exit(1)
