...
public class RequestRepository extends Repository {
    ...
    public BigInteger getLastSerialNumberInRange(BigInteger min, BigInteger max) throws EBaseException {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            // if request ID in range (min <= request ID <= max) => return request ID
            if ((requestID.compareTo(min) == 0 || requestID.compareTo(min) == 1)
                    && (requestID.compareTo(max) == 0 || requestID.compareTo(max) == -1)) {
