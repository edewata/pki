 ...
 */
public class SDHostFindCLI extends CommandCLI {

    public SDHostFindCLI(CLI parent) {
        super("find", "Find security domain hosts", parent);
    }
        ...
        try {
            ...
            for (SecurityDomainSubsystem sub : domainInfo.getSubsystems().values()) {
                for (SecurityDomainHost host : sub.getHosts().values()) {
                    SDHostCLI.printSecurityDomainHost(host);
                }
            }
