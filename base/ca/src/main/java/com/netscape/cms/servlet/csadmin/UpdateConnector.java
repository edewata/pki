...
/**
 * This servlet creates a KRA connector and a user with the KRA's subsystem cert.
 *
 * In a normal KRA installation pkispawn will request a subsystem cert from the CA
 * using the caInternalAuthSubsystemCert/caECInternalAuthSubsystemCert profile. These
 * profiles have a SubsystemGroupUpdater which will create a KRA subsystem user in the
 * CA with the new cert. This user later can be used by pkidestroy to remove the KRA
 * connector from the CA when the KRA is uninstalled.
 *
 * However, there are other profiles (e.g. CMC) that do not have a SubsystemGroupUpdater
 * so the KRA subsystem user will not be created. Instead, this class will now create
 * the user.
 */
