...
public class AuthorityMonitor implements Runnable {
    ...
    public void init() throws Exception {
        ...
        if (!foundHostCA) {
            ...
            engine.registerAuthority(engine.addHostAuthorityEntry(), hostCA);
        }
    }
    ...
    private synchronized void handleMODDN(DN oldDN, LDAPEntry entry) throws Exception {
        ...
        if (wasMonitored && !isMonitored) {
            ...
            if (attr != null) {
                ...
                engine.unregisterAuthority(aid);
            }
    ...
    private synchronized void handleDELETE(LDAPEntry entry) {
        ...
        if (engine.deletedNsUniqueIds.remove(nsUniqueId)) {
    ...
    public synchronized void readAuthority(LDAPEntry entry) throws Exception {
        ...
        if (engine.deletedNsUniqueIds.contains(nsUniqueID)) {
        ...
        if (authorityDN.toString().equals(hostCA.getX500Name().toString())) {
            ...
            engine.registerAuthority(authorityID, hostCA);
        ...
        BigInteger knownEntryUSN = engine.entryUSNs.get(authorityID);
        ...
        try {
            ...
            engine.registerAuthority(authorityID, ca);
            engine.entryUSNs.put(authorityID, newEntryUSN);
            engine.nsUniqueIds.put(authorityID, nsUniqueID);
